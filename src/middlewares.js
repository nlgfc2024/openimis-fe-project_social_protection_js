...
const projectBeneficiariesMiddleware = (store) => (next) => (action) => {
  ...
  if (isProjectBeneficiaryRequest && fetchAllForProjectId) {
    ...
    const orderBy = isGroup
      ? 'orderBy: ["groupBeneficiary__group__code"]'
      : 'orderBy: ["beneficiary__individual__last_name", "beneficiary__individual__first_name"]';
    ...
    const payloadField = isGroup ? 'groupBeneficiaryProjectEnrollment' : 'beneficiaryProjectEnrollment';
